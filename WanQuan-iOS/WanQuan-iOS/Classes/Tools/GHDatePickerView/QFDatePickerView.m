...
@property (nonatomic, assign) BOOL isWhetherToday;
...
- (instancetype)initDatePackerWithResponse:(void (^)(NSString *))block  isToday:(BOOL)isWhetherToday
{
    ...
    self.isWhetherToday = isWhetherToday;
    [self setViewInterface:isWhetherToday];
    if (block) {
        backBlock = block;
    }
    return self;
...
- (void)setViewInterface:(BOOL)isWhetherToday {
    //获取当前时间 （时间格式支持自定义）
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    if (component == 0) {
        ...
        if (row == (self.isWhetherToday == YES ? yearArray.count - 1 : yearArray.count)) {//至今的情况下,月份清空
