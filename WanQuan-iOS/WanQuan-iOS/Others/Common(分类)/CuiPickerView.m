...
- (id)init {
    if (self = [super init]) {
       ...
       // self.pickerView=[[UIPickerView alloc]initWithFrame:CGRectMake(0, 40, [UIScreen mainScreen].bounds.size.width, 180)];
        ...
        [cancelButton addTarget:self action:@selector(hiddenPickerViewBtn) forControlEvents:UIControlEventTouchUpInside];
...
- (void)hiddenPickerViewBtn {
    [UIView animateWithDuration:0.7f animations:^{
        self.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, self.frame.size.width, self.frame.size.height);
    } completion:^(BOOL finished) {
        [[NSNotificationCenter defaultCenter] postNotificationName:WQhiddenPickerView object:self];
        // self.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, self.frame.size.width, self.frame.size.height);
    }];
