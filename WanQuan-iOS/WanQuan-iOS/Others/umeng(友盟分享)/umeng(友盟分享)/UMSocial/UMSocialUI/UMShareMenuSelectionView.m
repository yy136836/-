...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.columnSpace = 5;
        self.lineSpace = 10;
        self.edgeInsets = UIEdgeInsetsMake(20,0,20,0);
        self.lineCount = UMSocial_Max_Row_Count;
        self.columnCount = UMSocial_Item_Count_PerRow;
        CGFloat UMSocial_Item_Width = kScaleX(40)+21;
...
//        }
      ...
      //  [self creatCancelButton];
        ...
        NSMutableArray *arrayM = [UMSocialManager defaultManager].platformTypeArray;
        NSString *platName = [WQSingleton sharedManager].platname;
        NSMutableArray *platArray= [[NSMutableArray alloc]initWithArray:arrayM];
        [platArray addObject:@(WQCopyLink)];
        if ([platName isEqualToString:@"WQ"]) {//分享到万圈
            [platArray addObject:@(WQQuanZi)];
        }else if([platName isEqualToString:@"WQHY"]){
            [platArray addObject:@(WQFriend)];
        }else if ([platName isEqualToString:@"WXQY_INVITE"]){
            [platArray addObject:@(WQFriend)];
            [platArray addObject:@(WQFriedd_INVITE)];

        }
        for (NSNumber *platformType in platArray) {
...
//            pageView.layer.cornerRadius = UMSocial_Menu_CornerRadius;
        ...
        self.contentViewWithIndex = ^(UMSocialShareScrollView *scrollView, NSInteger index, DJXScrollSubViewLocation location){
           ...
           // pageSize.width = pageSize.width - UMSocial_Page_Space*2;//设置页之间的间隔
            pageSize.width = pageSize.width;//设置页之间的间隔
...
//        self.columnSpace = 30;// (selfFrame.size.width - UMSocial_Page_Space*2 - self.edgeInsets.left - self.edgeInsets.right - self.columnCount*self.itemSize.width)/(self.columnCount - 1);
//    }else{
//        //如果列数少于3个 则重新布局
//        self.columnSpace = (selfFrame.size.width - self.columnCount*self.itemSize.width)/(self.columnCount + 1);
//        //修改偏移量
...
//    }
    if (self.columnCount <5) {
        self.columnSpace = (kScreenWidth-self.columnCount*self.itemSize.width)/(self.columnCount+1);
    }
...
- (CGRect)itemFrameWithIndex:(NSInteger)index
{
    ...
    rect.origin.x =  column * (self.columnSpace + self.itemSize.width) + self.edgeInsets.left+self.columnSpace;
    rect.origin.y = line * (self.lineSpace + self.itemSize.height)+self.edgeInsets.top;
...
- (NSMutableDictionary *)dictWithPlatformName:(NSNumber *)platformType
{
    ...
    switch (platformType_int) {
        ...
        case UMSocialPlatformType_WechatSession:
            imageName = @"weixinhaoyou";
            platformName = UMLocalizedString(@"wechat",@"微信好友");
            break;
        case UMSocialPlatformType_WechatTimeLine:
            imageName = @"weixinpengyouquan";
            platformName = UMLocalizedString(@"wechat_timeline",@"朋友圈");
            break;
    ...
    cell.tapActionBlock = ^(NSInteger index){
        // 复制链接的响应事件
        if (index == 100000) {
            [[NSNotificationCenter defaultCenter] postNotificationName:WQSharCopyLink object:self];
        }
