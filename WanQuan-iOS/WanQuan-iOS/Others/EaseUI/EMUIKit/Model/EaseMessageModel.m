...
- (instancetype)initWithMessage:(EMMessage *)message
{
    ...
    if (self) {
        ...
        if(ext){
            NSString *userName = [ext objectForKey:@"from_name"];
            NSString *userPic = [ext objectForKey:@"from_pic"];
            
            if([message.from isEqualToString:[EMClient sharedClient].currentUsername] && message.direction == EMMessageDirectionSend){
                userPic = [ext objectForKey:@"from_pic"];
                userName = [ext objectForKey:@"from_name"];
            }else{
                
            }
            
            _nickname = userName ? userName : @"";
            if(userPic){
                _avatarURLPath = [imageUrlString stringByAppendingString:userPic];
            }
        }
        ...
        NSLog(@"direction=%d,userPic=%@,userName=%@,content=%@", message.direction, _avatarURLPath, _nickname, self.text);
    }
