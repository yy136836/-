                        ...
                        model:(id<IMessageModel>)model
{
    ...
    if (self) {
        ...
        if (model.bodyType == EMMessageBodyTypeImage) {
            [self setSendBubbleBackgroundImage:[[UIImage imageNamed:@"tupianbeijingyou"] stretchableImageWithLeftCapWidth:5 topCapHeight:35]];
            [self setRecvBubbleBackgroundImage:[[UIImage imageNamed:@"tupianbeijingzuo"] stretchableImageWithLeftCapWidth:35 topCapHeight:35]];
        } else {
            [self setSendBubbleBackgroundImage:[[UIImage imageNamed:@"ziduihuakuang"] stretchableImageWithLeftCapWidth:5 topCapHeight:35]];
            [self setRecvBubbleBackgroundImage:[[UIImage imageNamed:@"baiduihuakuang"] stretchableImageWithLeftCapWidth:35 topCapHeight:35]];
        }
...
- (void)layoutSubviews
{
    ...
    if (self.model.message.ext[@"group"]) {
        _bubbleView.backgroundImageView.image = self.model.isSender ? [UIImage imageNamed:@"fenxiangqundizuo"] : [UIImage imageNamed:@"fenxiangqundiyou"];
    }
...
- (void)configureSendLayoutConstraints
{
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.avatarView attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeRight multiplier:1.0 constant:-15]];
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.nameLabel attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.avatarView attribute:NSLayoutAttributeLeft multiplier:1.0 constant:-15]];
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.bubbleView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.nameLabel attribute:NSLayoutAttributeBottom multiplier:1.0 constant:6]];
...
- (void)configureRecvLayoutConstraints
{
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.nameLabel attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:self.avatarView attribute:NSLayoutAttributeRight multiplier:1.0 constant:15]];
...
//        [self.avatarView sd_setImageWithURL:[NSURL URLWithString:model.avatarURLPath] placeholderImage:model.avatarImage];
        [self.avatarView yy_setImageWithURL:[NSURL URLWithString:model.avatarURLPath] placeholder:model.avatarImage];
    } else {
        self.avatarView.image = model.avatarImage;
    }
    _nameLabel.text = model.nickname;
    
    __weak EaseBaseMessageCell *weakSelf = self;
