...
{
    self = [self initWithFrame:frame horizontalPadding:15 verticalPadding:10 inputViewMinHeight:30 inputViewMaxHeight:150 type:EMChatToolbarTypeGroup];
    if (self) {
...
{
    self = [self initWithFrame:frame horizontalPadding:15 verticalPadding:10 inputViewMinHeight:30 inputViewMaxHeight:150 type:type];
    if (self) {
...
- (void)_setupSubviews
{
    ...
    //input textview
    _inputTextView = [[EaseTextView alloc] initWithFrame:CGRectMake(10, self.verticalPadding, self.frame.size.width - 10 * 2, self.frame.size.height - self.verticalPadding * 2)];
...
//    [styleChangeButton setImage:[UIImage imageNamed:@"EaseUIResource.bundle/chatBar_record"] forState:UIControlStateNormal];
    [styleChangeButton setImage:[UIImage imageNamed:@"maikefeng"] forState:UIControlStateNormal];

    [styleChangeButton setImage:[UIImage imageNamed:@"jianpan"] forState:UIControlStateSelected];
    ...
    [self.faceButton setImage:[UIImage imageNamed:@"xiaolian"] forState:UIControlStateNormal];
    [self.faceButton setImage:[UIImage imageNamed:@"xiaolian"] forState:UIControlStateHighlighted];
    ...
    [self.moreButton setImage:[UIImage imageNamed:@"chat_tianjia"] forState:UIControlStateNormal];
    [self.moreButton setImage:[UIImage imageNamed:@"chat_tianjia"] forState:UIControlStateHighlighted];
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView
{
    ...
    if ([self.delegate isKindOfClass:[EaseMessageViewController class]]) {
        EaseMessageViewController * vc = (EaseMessageViewController * )self.delegate;
        if (vc.bidFinished) {
            [WQAlert showAlertWithTitle:@"" message:@"订单已完成，请勿在此继续交流" duration:1.5];
            return NO;
        }
        

        if (vc.tempChatBiding) {
            [WQAlert showAlertWithTitle:@"" message:@"已接单，请在订单处交流" duration:1.5];
            return NO;
