               ...
               otherConfig:(NSDictionary *)otherConfig
{
    ...
    if (otherConfig && [otherConfig objectForKey:kSDKConfigEnableConsoleLogger]) {
        options.enableConsoleLog = YES;
    }
    
    EMError *error = [[EMClient sharedClient] initializeSDKWithOptions:options];
    if(error){
        NSLog("EaseMob error:%@", error.errorDescription);
    }
...
+ (EMMessage *)sendImageMessageWithImage:(UIImage *)image
                             ...
                             messageType:(EMChatType)messageType
                              messageExt:(NSDictionary *)messageExt
{
    NSData *data = UIImageJPEGRepresentation(image, 7);
...
- (void)logOutForced:(BOOL)forced  shouldReLogin:(BOOL)reLogin {
    if ([EMClient sharedClient].isLoggedIn) {
        [[EMClient sharedClient] logout:YES completion:^(EMError *aError) {
            if (forced) {
                while (aError) {
                    [self logOutForced:forced shouldReLogin:reLogin];
                    return;
                }
                if (reLogin) {
                    [self loginFroced:YES];
                }
            }
        }];
    }
